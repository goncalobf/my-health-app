...
- `src/lib/macro-targets.ts`, `nutrition-phase.ts`, `progressive-overload.ts`, and `training-plan.ts`: deterministic health/training rules. Keep calculations here rather than delegating them to a model.
...
- Workouts support routines, a fixed Monday-Sunday schedule, guided sessions, rest timers, RIR, double-progression recommendations, fatigue check-ins, and deload guidance.
