...
export async function GET() {
  const rows = await db.select().from(mealTemplates).orderBy(desc(mealTemplates.createdAt));
...
export async function POST(req: Request) {
  ...
  if (body.logTemplateId) {
    const [template] = await db.select().from(mealTemplates).where(eq(mealTemplates.id, Number(body.logTemplateId)));
  ...
  const [row] = await db.insert(mealTemplates).values({ name, itemsJson: JSON.stringify(items) }).returning();
