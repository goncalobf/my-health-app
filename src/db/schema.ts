...
export const settings = pgTable("settings", {
  ...
  // Food search order and localized names are user preferences. "both" keeps
  // Portuguese and Swiss official sources at equal regional priority.
  foodRegion: text("food_region").notNull().default("both"),
  foodLanguage: text("food_language").notNull().default("pt"),
...
// Shared, versioned national food-composition records. These contain no user
// data. Consumer-package barcodes stay in Open Food Facts; documented Swiss
// branded exceptions (mostly mineral water/alcohol) retain their official ID.
export const foodCatalogItems = pgTable(
  "food_catalog_items",
  {
    ...
    category: text("category"),
    basisQuantity: real("basis_quantity").notNull().default(100),
...
export const foodCatalogNames = pgTable(
  "food_catalog_names",
  {
    ...
    foodId: integer("food_id")
      .notNull()
      .references(() => foodCatalogItems.id, { onDelete: "cascade" }),
    language: text("language").notNull(),
    name: text("name").notNull(),
    synonyms: text("synonyms"),
    // Diacritic-free, lower-case text generated by the importer. It permits
    // consistent matching without depending on a database collation.
...
export type FoodCatalogName = typeof foodCatalogNames.$inferSelect;
