...
export default function EasyRunRecording({
  session,
  onDone,
}: {
  ...
  const distKm = dist ? parseFloat(normalizeDecimalInput(dist)) : 0;
  const paceSecPerKm = distKm > 0 && elapsed > 0 ? elapsed / distKm : 0;
  const paceStr =
    paceSecPerKm > 0
    ...
    return (
      <div className="flex min-h-[80dvh] flex-col gap-5 pb-6">
        <div className="card p-5 text-center">
          ...
          {paceStr && <p className="mt-1 text-sm text-muted">{paceStr}</p>}
        </div>

        <div className="card flex flex-col gap-4 p-5">
          <h2 className="label">Log your run</h2>
  ...
  return (
    <div className="flex min-h-[80dvh] flex-col items-center justify-center gap-8">
      <div className="text-center">
        <p className="label mb-2">Easy run · in progress</p>
